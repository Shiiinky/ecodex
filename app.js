...
async function identifyWithApi(dataUrl){
  try{
    ...
    if(out?.mappedKey && SPECIES[out.mappedKey]) return { key:out.mappedKey, score:out.score||null };
    if(out?.species){
      const s=out.species;
      const key=s.slug||(s.common_name_fr||out.label||'animal').toLowerCase().replace(/[^a-z0-9]+/g,'_');
      if(SPECIES[key]) return { key, score:out.score||null };
...
function offlineGuessFromFile(file){
  ...
  const map=[[/chat|cat|felis|tabby/,'cat'],[/chien|dog|canis/,'dog'],[/herisson|hedgehog/,'hedgehog'],[/mesange|tit/,'blue_tit'],[/libellule|dragonfly/,'dragonfly'],[/coccinelle|ladybird|ladybug/,'ladybird'],[/rouge.?gorge|robin/,'robin'],[/renard|fox/,'fox'],[/ecureuil|squirrel/,'squirrel'],[/grenouille|frog/,'frog'],[/chamois/,'chamois'],[/loutre|otter/,'otter'],[/flamant|flamingo/,'flamingo'],[/abeille|bee/,'honeybee'],[/sanglier|boar/,'boar'],[/chevreuil|deer/,'deer'],[/canard|duck/,'duck'],[/hirondelle|swallow/,'swallow'],[/heron/,'heron'],[/pie|magpie/,'magpie']];
...
async function processImage(dataUrl, file){
  ...
  if(!res){ showWarn("Identification auto indisponible.<br>Utilise un <strong>exemple</strong> ou renomme le fichier (ex. <code>chat.jpg</code>).<br>Pour Google Vision : configure <code>GOOGLE_VISION_API_KEY</code> sur Netlify."); setBusy(false); return; }
...
if('serviceWorker' in navigator){
  window.addEventListener('load', ()=>navigator.serviceWorker.register('./sw.js?v=12').catch(console.error));
}
