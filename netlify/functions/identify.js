      ...
      if (!apiKey) {
        ...
        if (r.ok) {
          ...
          if (obj.length){
            ...
            const preferred = ['Cat','Dog','Bird','Hedgehog','Mammal','Animal','Insect','Butterfly','Fish','Reptile','Amphibian'];
            const hit = obj.find(o => preferred.includes(o.name)) || obj[0];
            if ((hit.score||0) >= MIN_CONFIDENCE*0.9){
              best = { label: hit.name, score: hit.score };
            }
          }
